<template>
	<canvas
		...
		@click="canvasClick"
	></canvas>
	<div v-if="textBoxShowing" :style="textBoxStyle" id="text-box" class="position-absolute">
		<button
...
<script setup>
	import { ref, computed, watch, useTemplateRef, onMounted, onUnmounted, nextTick } from 'vue'
	...
	const { clearBoard, currentColor, fillBoard } = storeToRefs(boardStore)
	...
	const eraserMoveStroke = rafThrottle((event) => {
		if (beginDrawing) {
			let eventPositionX
			let eventPositionY
			if (event.type === 'touchmove') {
	...
	const okTextBtnClick = () => {
		if (textBoxContentRef.value.textContent) {
			...
			pathData.actions.push({ type: 'fillText', x: printPlaceX + 8, y: printPlaceY + 12 })
			emit('updatePath', { ...pathData })
			pathData = { actions: [] }
			textBoxShowing.value = false
		}
	...
	/* 視窗調整時，更新textbox位置 */
	const updateTextBoxPlace = debounce(() => {
	...
	watch(
		() => boardMode,
		(newValue) => {
			if (textBoxShowing.value && newValue !== 'outer') {
				textBoxShowing.value = false
				resetClickCount()
			}
		},
